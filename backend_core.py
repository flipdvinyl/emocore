...
if not GEMINI_API_KEY:
    raise RuntimeError(
        "GEMINI_API_KEY is not set. Please create a .env file with GEMINI_API_KEY=<your key>."
    )
...
def generate_from_payload(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not base_text:
        return {"status": 400, "body": {"text": "", "error": "missing_base_text"}}
    if target_length <= 0:
    ...
    if analysis_only:
        ...
        return {"status": 200, "body": {"text": base_text, "emotion": emotion, "language": language}}
    ...
    try:
        ...
        return {"status": 200, "body": {"text": text, "emotion": emotion, "language": language}}
        ...
        return {"status": status, "body": {"text": base_text, "error": detail}}
